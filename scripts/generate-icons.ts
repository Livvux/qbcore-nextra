...
async function generateIcons(): Promise<void> {
  ...
  try {
    ...
    fs.writeFileSync(path.join(publicDir, 'icon-192.svg'), appleIconSVG.replace(/180/g, '192'))
    fs.writeFileSync(path.join(publicDir, 'icon-512.svg'), appleIconSVG.replace(/180/g, '512'))
